...
public final class WorksheetReader {
  ...
  public interface WorksheetBuilder<T> {
    ...
    WorksheetBuilder<T> addNewSheet(String name);
  ...
  public static <T> T read(WorksheetBuilder<T> builder, Readable readable)
    ...
    while (scan.hasNext()) {
      ...
      if (scan.hasNext("%.*")) {
        String newSheetName = scan.nextLine();
        newSheetName = newSheetName.equals("") ? scan.nextLine() : newSheetName;
        newSheetName = newSheetName.substring(1);
        builder = builder.addNewSheet(newSheetName.replace(" ", ""));
        //scan.nextLine();
        continue;
      }
